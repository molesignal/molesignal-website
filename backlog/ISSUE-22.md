...
- 2026-06-02 10:21:01 set status=in_review

## 代码审查（code-reviewer · 2026-06-02）

### 结论：**PASS，建议放行到独立 QA 验证**。必改问题 0；建议改 0；可选/观察 2。

逐文件审查 `git diff main...feature/ISSUE-22` 全部改动，对照 06 架构评估与 AC1–AC7：

#### 正确性 ✅
- **AC2 死链守卫**（`app/[locale]/blog/[slug]/page.tsx:56-59`）：逻辑正确。`const exists = Boolean(getPostBySlug(slug))` → `href = exists ? /blog/${slug} : "/blog"`、`label` 同步二选一。`getPostBySlug` 第 9 行本就 import（EN 分支第 80 行也用），零新依赖。不存在 slug 回退到 `/blog`，根治了原「`/zh/blog/<missing>` 链到 `/blog/<missing>`(404)」死链。SSG 不受影响（`generateStaticParams` 仅产 EN slug，ZH 走动态渲染）。
- **列表页**（`app/[locale]/blog/page.tsx:45`）：裸路径 `/blog →` 改 `{t("zhReadEnglish")}` 描述性文本；列表链接本就指向 `/blog` 无死链风险，改动安全。
- **sitemap 死代码清理**（`app/sitemap.ts`）：`const fallback = routing.locales; void fallback;` 及 `routing` import 一并删除。已 grep 确认 `routing` 在 sitemap.ts 内无其他引用 → 无悬空引用、无 eslint unused。blog 条目仍 EN-only 无 zh alternates（符合 AC5）。

#### 契约一致 ✅
- 未碰 `content/blog.ts` 契约 / frontmatter schema / `i18n/routing.ts`，严格守住本期 EN-only scope（红线②未触碰）。
- i18n parity：`messages/{en,zh}.json` 各 +2 键（`zhReadEnglish`/`zhReadEnglishPost`），`zhUnavailable` 改纯通知文案。**实测 `lint:i18n` EN 518 = ZH 518 OK**，键结构对齐。

#### 可维护性 / 风格 ✅
- 注释清晰说明 AC2 守卫意图；token 完全复用（`Section`/`text-display-md`/`text-primary` 等），无新类名/组件。
- E2E（`issue22-t21-blog-zh-notice.spec.ts`）质量高：3 条覆盖 AC1 / AC2-存在 / AC2-不存在；对「裸 EN 路径真 404」断言用 `request.newContext()` cookie-clean 上下文规避共享 `NEXT_LOCALE=zh` cookie 把 EN 路径重定向回 ZH 通知页的陷阱 —— 这是个容易踩的坑，处理得当。
- Commit 符合 Conventional Commits：`feat(ISSUE-22): ...`。

#### 复核门禁（审查员独立跑，非仅采信开发自测）
- `tsc --noEmit`：**rc=0** ✅
- `lint:i18n`：**EN 518 = ZH 518，parity OK** ✅
- `eslint`：**0 errors**；有 5 warnings 但**全部在本工单范围外的既有文件**（如 TOC 组件 `react-hooks/set-state-in-effect`），本次改动零新增 warning，无回归。

#### 可选/观察（非阻塞，不必返工）
1. 开发记录写「eslint 0」，实际是 **0 errors + 5 既有 warnings**；warnings 与本工单无关，属轻微措辞不精确，非代码问题。
2. `build` / `test:e2e` / `lint:links` 三项较重，审查环未重跑，采信开发自测（76 passed / 0 死链 / build rc=0）；**交独立 QA 复跑确证**（符合三环不省原则）。

**放行建议**：代码层面无必改项，逻辑/契约/风格均达标 → 移交 qa-automation 独立验证 AC1–AC7（重点复跑 build + test:e2e + lint:links，并实证不存在 slug 回退）。
- 2026-06-02 10:22:51 set status=verifying

## QA 验证结果（qa-automation · 2026-06-02 · 独立复跑）

### VERDICT: PASS

独立起服务、真跑全量门禁 + 接口/HTML 实证 + E2E，逐条核验 AC1–AC7（AC8 为翻案占位本期不验）。**全绿，无阻断缺陷，建议放行合并。**

#### 执行环境与启动方式
- Node v23.6.1（nvm）/ pnpm 11.5.0；`pnpm build` 生产构建 rc=0。
- E2E：Playwright 自管 `next start -p 3210`（生产构建），76 用例全过。
- 链路/接口实证：后台 `next start -p 3000`，curl + `lint:links` 爬全站；测毕已 kill，端口 3000/3210 均释放、无僵尸。

#### 门禁结果（全部独立复跑，非采信开发自测）
| 门禁 | 结果 |
|---|---|
| `pnpm typecheck`（tsc --noEmit） | rc=0 ✅ |
| `pnpm lint`（eslint） | 0 errors（5 warnings 全在工单外既有文件 `components/toc.tsx`，本工单零新增）✅ |
| `pnpm lint:i18n` | EN 518 = ZH 518，parity OK ✅ |
| `pnpm a11y:contrast` | 26/26 AA 通过 ✅ |
| `pnpm build` | rc=0，blog 两篇 SSG、`/zh/blog*` 动态渲染 ✅ |
| `pnpm test:e2e` | **76 passed**（含本工单 issue22-t21 三条）✅ |
| `pnpm lint:links` | **0/33 失败**，`/zh/blog` 200，全站 blog 链路无死链 ✅ |

#### AC 逐条实证（curl 真实输出）
- **AC1** ✅ `/zh/blog` 渲染通知 `Blog 目前仅支持英文版` + 描述性链接 `阅读英文版 Blog →`（href `/en/blog`，307→`/blog` 200，非死链）。
- **AC2** ✅（核心死链守卫）
  - `/zh/blog/why-parquet-for-three-signals`（存在）→ `阅读该文章英文版 →` 深链 `/en/blog/why-parquet-for-three-signals`（200）。
  - `/zh/blog/does-not-exist-xyz`（不存在）→ 回退 `阅读英文版 Blog →` 指向 `/en/blog`；页面 **无任何** `/blog/does-not-exist-xyz` 链接。
  - 死链反证（cookie-clean）：裸 `GET /blog/does-not-exist-xyz` = **404**；`GET /blog/why-parquet-for-three-signals` = **200** → 守卫确实根治了原潜在死链。
- **AC3** ✅ `messages/{en,zh}.json` 均有 `zhUnavailable`/`zhReadEnglish`/`zhReadEnglishPost`，语义对齐，`lint:i18n` parity 518=518。
- **AC4** ✅ 复用 `Section`/`text-display-md`/`text-primary` token，链接为描述性可达文本（非裸 URL）；`a11y:contrast` 26/26 AA。
- **AC5** ✅ `sitemap.xml`：`zh/blog` 出现 **0** 次；blog 三条 `<loc>` 均 **NO_ALTERNATE**（无 `xhtml:link` hreflang），不向 SEO 宣告不存在的 ZH blog。
- **AC6** ✅ LocaleSwitcher EN→ZH 落 `/zh/blog*` 通知页 2xx（`lint:links` /zh/blog 200 + E2E「key internal routes 2xx EN+ZH」「locale switch 保留滚动」全过）。
- **AC7** ✅ 不回归：T15 富文本（headings/lists/inline code/Shiki + CodeBlock 复制）、`getRelatedPosts` 相关文章 E2E 全过；check/build/test:e2e/lint:links 全绿。

#### 缺陷清单
无（0 阻断 / 0 一般 / 0 轻微）。

#### 回归套件位置与重跑命令
- 新增守卫：`tests/e2e/issue22-t21-blog-zh-notice.spec.ts`（3 条：AC1 / AC2-存在 / AC2-不存在回退）。
- 重跑：`pnpm test:e2e`（全量 76）｜针对本工单 `pnpm exec playwright test issue22-t21`｜链路 `pnpm lint:links`（需另起 `next start -p 3000`）｜门禁 `pnpm check`。

**放行建议**：PASS，可 `git merge --no-ff` 合并回 main。
- 2026-06-02 10:22:51 set status=verifying
- 2026-06-02 10:26:59 set status=closed
- 2026-06-02 10:26:59 引擎周期#23: QA PASS, 关闭
