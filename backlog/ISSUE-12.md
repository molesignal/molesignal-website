...
- 2026-06-02 06:49:53 set status=verifying

## QA 验证结果（qa-automation, 2026-06-02）：通过

**VERDICT: PASS**

**方式**：production build (`pnpm build` rc=0 → 全部迁移 token 编译成功) → `next start -p 3210` 真实生产渲染 → Playwright 真 Chromium 驱动断言计算样式 + 截图。中心风险（token 编译成 nothing 像 ISSUE-10/11）逐项用 computed style 证伪。

**新增 E2E 套件**：`tests/e2e/issue12-t08b-visual.spec.ts` — **9/9 passed (8.1s)**
- ① TopNav 高度 = 56px（量 `.h-nav` boundingBox=56）✅；brand wordmark fontFamily 含 mono ✅
- ① active 导航项（/why）`border-bottom-width≥2px` + 底边 brand 色已上漆 + **背景未上漆**（证明是下划线非填充 pill）✅
- ① 滚动 >80px 后 header `backdrop-filter` 含 blur + 背景上漆（surface/90）✅
- ② Footer 四列标题 count=4 ✅；Discord/Twitter `aria-disabled` count=2（T04 未破坏）✅；链接默认 muted 色已上漆 ✅
- ③ Pill brand 变体（/start）真有 `bg-brand-dim` 填充 + brand 边框(≥1px) + brand teal 文字（g>r,g>b）✅；TopNav brand CTA 真有 primary 填充 ✅
- ④ Footer Download→`/start#install`、GitHub `target=_blank rel=noreferrer` ✅

**回归（无破坏）**：
- T04 死链 `tests/e2e/issue3-t04-deadlinks.mjs`：**13/13 passed**（Discord/Twitter 非锚点+aria-disabled、Download→/start#install、helm/binary v1.0 notice 均未动）
- UI token `tests/e2e/issue5-ui-tokens.mjs`：**29/29 passed**（brand=teal/amber、Geist Mono、radius 等 token 双主题全绿，nav 高度变更未波及）

**截图证据**：`test-results/issue12/{topnav-scrolled-why-active,footer,start-hero-brand-pill}.png`（截图显示 mono wordmark、Why 项 teal 下划线、滚动后 blur 背景、四列 footer + SOON 禁用徽章）

**重跑命令**：
```
pnpm build && pnpm exec playwright test tests/e2e/issue12-t08b-visual.spec.ts
# 回归(需先 next start -p 3210):
